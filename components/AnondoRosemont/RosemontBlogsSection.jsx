...
function BlogCard({ blog, index, compact = false }) {
  return (
    <Link
      ...
      className="group block overflow-hidden border border-secondary/15 bg-white shadow-xl shadow-secondary/10 transition duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-secondary/15"
    >
      ...
      <div className="p-7">
        ...
        {!compact && (
          <ul className="mt-7 grid gap-3 border-t border-secondary/15 pt-6 md:grid-cols-3">
            {blog.points.map((point) => (
              <li
                key={point}
                className="flex gap-3 text-sm leading-7 text-[#6b5d57]"
              >
        ...
        <div className="mt-7 flex items-center justify-between gap-5 border-t border-secondary/15 pt-6">
          <span className="text-xs font-extrabold uppercase tracking-[0.2em] text-secondary/55">
            Read Full Article
          </span>
          <span className="h-px w-16 origin-left scale-x-[0.35] bg-secondary transition-transform duration-700 group-hover:scale-x-100" />
        </div>
      </div>
    </Link>
  );
      ...
      <div className="mx-auto max-w-[1400px] px-5">
        <div className="grid gap-10 border-b border-secondary/15 pb-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <h2 className="max-w-4xl text-4xl font-semibold leading-tight text-secondary md:text-5xl lg:text-6xl">
          ...
          <div>
            ...
            {!page && (
              <Link
                href="/blogs"
                className="group mt-7 inline-flex items-center gap-4 bg-secondary px-7 py-4 text-sm font-extrabold uppercase tracking-[0.14em] text-white transition duration-500 hover:bg-[#241818]"
              >
                View All Blogs
