  ...
  return (
    <section className="bg-[#f8f3ec] py-16 text-[#241818] md:py-24 lg:py-32">
      <div className="mx-auto max-w-[1400px] px-5">
        <div className="mb-12 grid gap-8 border-b border-secondary/15 pb-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
          ...
          {residentialComponents.map((item, index) => {
            ...
            return (
              <article
                id={meta.slug}
                key={item.title}
                className={`group scroll-mt-28 border border-secondary/15 p-7 shadow-xl shadow-secondary/10 transition duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-secondary/18 ${
                  index === 0
                    ? "bg-secondary text-white lg:col-span-2"
                ...
                <div className="mb-8 flex items-center justify-between gap-6">
                  <p
                    className={`text-xs font-extrabold uppercase tracking-[0.22em] ${
                      index === 0 ? "text-white/55" : "text-secondary/55"
                    }`}
                  >
                    {meta.eyebrow}
                  </p>
